...
function Singlepost() {
  ...
  useEffect(() => {
    const fetchSinglePost = async () => {
      setProgress(0);
      const res = await axios.get(`https://mern-blog-app-backend-with-jwt.onrender.com/api/posts/${postId}`);
      // console.log(res);
      setProgress(30);
  ...
  const handleDelete = async () => {
    try {
      const res = await axios.delete(`https://mern-blog-app-backend-with-jwt.onrender.com/api/posts/delete/${postId}`, {
        headers: {
